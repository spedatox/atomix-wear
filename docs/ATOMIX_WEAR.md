...
**This repository** (`spedatox/atomix-wear`) — the Wear OS application, standalone
and self-contained, exactly as Ultron Wear is. It carries its own Gradle build,
its own CI and its own release pipeline.

**`spedatox/speda-mark6`** — everything server-side:
