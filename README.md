...
releasing is covered in [docs/RELEASING.md](docs/RELEASING.md). This file covers
building and running it.

The server side — the `/training/*` endpoints, the exercise catalog and the
`set_training_plan` skill — lives in `spedatox/speda-mark6` under
`packages/igor`. Phase 1 needs none of it.

> **Status: Phase 1, not yet compiled.** This has never been built — there was no
> JDK or Android SDK on the machine it was written on. It is structurally
> complete, and [`android-ci.yml`](.github/workflows/android-ci.yml) is the first
> thing that will actually compile it. Treat every claim below as intent until
> that workflow is green. See [Known unknowns](#known-unknowns).
...
Releases are automatic — push to `main` and CI publishes a signed APK and AAB.
See [docs/RELEASING.md](docs/RELEASING.md) for the one-time keystore setup.
...
- **The launcher icon is a placeholder.** Replace `ic_launcher_foreground.xml`
  with the real Atomix mark (it lives at `logos/atomix.svg` in speda-mark6).
...
Every source file carries an SPDX header; new files need one too.

A deliberate choice, matching Mark VI itself. Nothing this project depends on
requires copyleft — the exercise dataset is MIT, which is permissive — but
Mark VI is AGPL and this is part of it. (Ultron Wear is MIT; both are Mark VI
clients, so the licence is a per-project decision, not a consequence of what a
client is.)
